        ...
        if (cur_header % 2 == 0 && size <= cur_size) { // check if free
            ...
            if (cur_size >= rounded + 16) {
                ...
                *current = rounded + 1; // rounds up and sets the "busy" bit
                                        ...
                                        // the amount ht a user requested
            }
            else {
...
int main() {
    ...
    print_heap();
    int* e = my_malloc(8); // what should we expect? (change 12 to 8)
    print_heap();
}
...
my_malloc(12)
current = (-)
cur_header = 16 0b10000 0x10
rounded = 16
remaining = 16 - (16 + 8) = -8

why should we do +16 in the if statement?
header for busy block     how much extra space (remaining)
______________________________________________
| cur_size |   |   |   |   |  new_free | ... |
______________________________________________
            <---size--->    
which will take up some amount of space

            (-) *current = rounded + 1   b[0] b[1]
________________________________________________________________________________________
| 41 |   |   |   |   |   | 16 -> 17 -> 9 |   |   | 25 |   |   |   | 33 |   |   |   |    | 248 |  ...
________________________________________________________________________________________
